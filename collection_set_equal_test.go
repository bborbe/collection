...
var _ = Describe("SetEqual", func() {
	...
	Context("NewSetEqual with variadic constructor", func() {
		...
		It("creates set with multiple elements", func() {
			...
			multiSet := collection.NewSetEqual(user1, user2, user3)
		...
		It("handles duplicate elements correctly", func() {
			...
			dupSet := collection.NewSetEqual(user, user, user)
		...
		It("treats different users as separate elements", func() {
			...
			equalSet := collection.NewSetEqual(user1, user2)
			Expect(equalSet.Length()).To(Equal(2))
		})
	})
	...
	Context("String", func() {
		...
		It("returns string with multiple elements in sorted order", func() {
			user1 := User{Firstname: "Alice", Age: 25}
			user2 := User{Firstname: "Bob", Age: 30}
			user3 := User{Firstname: "Charlie", Age: 35}
			set.Add(user1)
	...
	Context("Each", func() {
		...
		It("iterates in insertion order (FIFO)", func() {
			user1 := User{Firstname: "Alice", Age: 25}
			user2 := User{Firstname: "Bob", Age: 30}
			user3 := User{Firstname: "Charlie", Age: 35}
			set.Add(user1, user2, user3)
			...
			order := make([]string, 0)
			err := set.Each(ctx, func(ctx context.Context, u User) error {
				order = append(order, u.Firstname)
				return nil
			})

			Expect(err).NotTo(HaveOccurred())
			Expect(order).To(Equal([]string{"Alice", "Bob", "Charlie"}))
		})
		...
		It("stops on first error", func() {
			...
			Expect(count).To(Equal(2)) // Alice, then Bob (stops)
		})
